...
('role-user-001', '普通用户', 'user', '可以使用六爻占卜功能', 1),
('role-vip-001', 'VIP用户', 'vip', '可以使用所有占卜功能和AI分析', 1)
...
SELECT
  CONCAT('rp-admin-', SUBSTRING(MD5(CONCAT(r.id, p.id)), 1, 25)) as id,
  r.id as role_id,
...
INSERT INTO role_permissions (id, role_id, permission_id)
SELECT
  CONCAT('rp-user-', SUBSTRING(MD5(CONCAT(r.id, p.id)), 1, 26)) as id,
  r.id as role_id,
  p.id as permission_id
FROM roles r
CROSS JOIN permissions p
...
-- ====================================
-- 6. 为VIP用户角色分配占卜相关所有权限
-- ====================================
INSERT INTO role_permissions (id, role_id, permission_id)
SELECT
  CONCAT('rp-vip-', SUBSTRING(MD5(CONCAT(r.id, p.id)), 1, 27)) as id,
  r.id as role_id,
  p.id as permission_id
FROM roles r
CROSS JOIN permissions p
WHERE r.role_code = 'vip' AND p.module = 'divination'
ON DUPLICATE KEY UPDATE role_permissions.created_at = role_permissions.created_at;
